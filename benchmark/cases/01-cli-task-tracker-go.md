...
- **specify**: 4 op stories + persistence; **every acceptance criterion is EARS-shaped**
  ...
  paired with an `If…then`, no vague terms, AND no solution-leakage (no technology/
  implementation nouns like Redis/endpoint/table/cache in any criterion's response)).
...
`If …, then …` · criteria that prescribe HOW (name a technology/endpoint/table) instead of
observable WHAT/WHY · clarify asking cosmetics or not persisting answers · plan over-engineering
